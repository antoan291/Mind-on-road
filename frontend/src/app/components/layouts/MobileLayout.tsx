...
export function MobileLayout() {
  ...
  const bottomNavItems = [
    ...
    { path: '/menu', icon: Menu, label: 'Меню', action: () => setMenuOpen(true) },
  ];
    ...
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg-page)' }}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          >
            <span className="text-xs font-semibold" style={{ color: '#ffffff' }}>KA</span>
          </div>
          <div>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50" style={{ background: 'var(--bg-page)' }}>
          ...
          <div className="p-4 space-y-2">
            <MenuLink to="/payments" label="Плащания" icon={<CreditCard size={20} />} onClick={() => setMenuOpen(false)} />
            <MenuLink to="/invoices" label="Фактури" onClick={() => setMenuOpen(false)} />
            <MenuLink to="/practical-lessons" label="Практика" onClick={() => setMenuOpen(false)} />
            <MenuLink to="/theory" label="Теория" onClick={() => setMenuOpen(false)} />
            <MenuLink to="/instructors" label="Инструктори" onClick={() => setMenuOpen(false)} />
            <MenuLink to="/vehicles" label="Автомобили" onClick={() => setMenuOpen(false)} />
