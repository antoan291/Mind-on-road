...
Този документ описва security architecture за продукта MindOnRoad.
...
### 4.3 Student portal credential bootstrap

За student portal достъп продуктът може да използва оперативното правило:

- телефонният номер на ученика става начално username;
- системата генерира временна парола при регистрация на ученика.

Това е допустимо само ако се спазят следните условия:

- телефонният номер се нормализира в един каноничен формат преди запис;
- временната парола е криптографски случайна и достатъчно дълга;
- в базата се пази само password hash, никога plaintext парола;
- временната парола се маркира като one-time credential;
- първият успешен login задължително изисква смяна на паролата;
- reset flow и повторно издаване на временна парола се audit-ват;
- администраторският интерфейс не трябва да показва стара временна парола след първоначалното издаване.

Причина:

- телефонният номер е удобен operational identifier, но не е достатъчна защита сам по себе си;
- сигурността идва от правилното управление на временната парола, forced password change и server-side session control.
