...
def render_pdf_pages_to_png_bytes(
  ...
  *,
  ...
  if not pdf_path.exists():
    raise InvalidDocumentError(f"PDF файлът не съществува: {pdf_path}")

  if pdf_path.suffix.lower() != ".pdf":
    raise InvalidDocumentError("Поддържа се само PDF вход.")

  try:
    document = fitz.open(pdf_path)
  except Exception as exc:  # pragma: no cover
    raise InvalidDocumentError(f"PDF файлът не може да бъде отворен: {pdf_path}") from exc
